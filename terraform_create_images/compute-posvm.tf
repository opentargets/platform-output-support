...
resource "google_compute_instance" "posvm" {
  ...
  provisioner "file" {
    content = templatefile("${local.path_source_postprocessing_scripts}/config.sh", {
      ...
      PATH_POSTPROCESSING_SCRIPTS_CLICKHOUSE                 = local.path_postprocessing_scripts_clickhouse,
      PATH_POSTPROCESSING_SCRIPTS_ELASTIC_SEARCH             = local.path_postprocessing_scripts_elastic_search,
      PATH_POSTPROCESSING_SCRIPTS_ENTRY_POINT_CLICKHOUSE     = local.path_postprocessing_scripts_entry_point_clickhouse,
      PATH_POSTPROCESSING_SCRIPTS_ENTRY_POINT_ELASTIC_SEARCH = local.path_postprocessing_scripts_entry_point_elastic_search,
      DATA_RELEASE_SEKELETON_PATH_OUTPUT_ROOT                = local.data_release_skeleton_path_output_root,
      DATA_RELEASE_SEKELETON_PATH_ETL_ROOT                   = local.data_release_skeleton_path_etl_root,
      DATA_RELEASE_SEKELETON_PATH_ETL_JSON_ROOT              = local.data_release_skeleton_path_etl_json_root,
      DATA_RELEASE_SEKELETON_PATH_ETL_PARQUET_ROOT           = local.data_release_skeleton_path_etl_parquet_root,
      DATA_RELEASE_PATH_SOURCE_ROOT                          = local.data_release_path_source_root,
      DATA_RELEASE_PATH_ETL_JSON                             = local.data_release_path_etl_json,
      DATA_RELEASE_PATH_ETL_PARQUET                          = local.data_release_path_etl_parquet,
      CLICKHOUSE_DOCKER_IMAGE                                = local.clickhouse_docker_image,
      CLICKHOUSE_DOCKER_IMAGE_VERSION                        = local.clickhouse_docker_image_version,
      POS_REPO_BRANCH                                        = var.config_repo_branch_pos,
      FLAG_POSTPROCESSING_SCRIPTS_READY                      = local.flag_postprocessing_scripts_ready,
