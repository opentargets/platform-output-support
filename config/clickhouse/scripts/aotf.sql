...
from (
        select
            row_id, disease_id as B, target_id as A, datatype_id, datasource_id, row_score, lower(disease_data) as B_search, lower(target_data) as A_search, ancestors
        from ot.associations_otf_log
            LEFT JOIN ot.disease_log ON ot.associations_otf_log.disease_id = ot.disease_log.id
    );
