...
output "pos_log_command" {
  value = {
    command = "gcloud compute ssh --zone ${google_compute_instance.posvm.zone} ${local.posvm_remote_user_name}@${google_compute_instance.posvm.name} --project open-targets-eu-dev --command 'journalctl -u google-startup-scripts.service -f'"
  }
